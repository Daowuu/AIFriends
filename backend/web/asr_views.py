...
from web.ai_settings_service import get_dashscope_runtime_config, get_public_dashscope_runtime_config
...
def speech_to_text_view(request):
    ...
    if runtime_config.get('source') == 'env':
        if not consume_system_api_quota(request, quota_type='voice'):
            return build_quota_exceeded_response(quota_type='voice', authenticated=True)
...
def demo_speech_to_text_view(request):
    audio_data = str(request.data.get('audio_data', '')).strip()
    ...
    if not runtime_config:
        return Response({
            'detail': '当前试玩语音识别未启用，请先在服务端配置可用的 ASR 或兼容 DashScope 的聊天配置。',
        }, status=status.HTTP_400_BAD_REQUEST)

    if runtime_config.get('source') == 'env':
        if not consume_system_api_quota(request, quota_type='voice'):
            return build_quota_exceeded_response(quota_type='voice', authenticated=False)
    ...
    try:
        ...
        return Response({'detail': f'试玩语音识别失败：{error}'}, status=status.HTTP_400_BAD_REQUEST)
